...
export function FormDialog<F extends FieldValues>(
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name, type }) => {
      props.onFormChange?.(form as any);
    });
  ...
  return (
    ...
    >
      <Form {...form}>
        <form onSubmit={e => runAsynchronouslyWithAlert(form.handleSubmit(onSubmit as any)(e))} className="space-y-4" id={formId}>
          {props.render(form as any)}
        </form>
      </Form>
    </ActionDialog>
  );

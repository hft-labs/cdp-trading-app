...
export const transactions = pgTable("transactions", {
    ...
    // Transaction details
    transaction_type: text("transaction_type").notNull(), // 'sent', 'received', 'swapped', 'smart_contract_executed', 'smart_contract_failed', 'sent_for_free'
    description: text("description").notNull(), // e.g., "To 0x2e06...ba83", "Wrapped Ether → USDC", "From 0x765d...bf85"
    
    // Value information
    value_usd: decimal("value_usd", { precision: 20, scale: 8 }), // Dollar amount, can be negative
    asset_amount: decimal("asset_amount", { precision: 30, scale: 18 }), // Token amount
    asset_symbol: text("asset_symbol"), // e.g., "USDC", "WETH", "ETH", "AERO"
    
    // Wallet information
    wallet_address: text("wallet_address").notNull(), // The wallet this transaction belongs to
    counterparty_address: text("counterparty_address"), // For sent/received transactions
    
    // Transaction metadata
    transaction_hash: text("transaction_hash"), // Blockchain transaction hash
    block_number: text("block_number"),
    gas_fee: decimal("gas_fee", { precision: 30, scale: 18 }), // For smart contract transactions
    ...
    // Network information
    network: text("network").default('ethereum'), // e.g., 'ethereum', 'polygon', etc.
});
